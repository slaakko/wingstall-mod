...
import lexer.parsing_exception;
import util;
    ...
    while (true)
    {
        ...
        if (state == -1)
        {
            ...
            else if (token.id == INVALID_TOKEN)
            {
                ...
                {
                    throw std::runtime_error("lexer::Lexer::NextToken(): error: invalid character '" + 
...
std::string Lexer::MappedErrorLines(const Token& token) const
{
    ...
    {
        return util::ToUtf8(ErrorLines(token));
    }
}
...
void Lexer::ThrowExpectationFailure(const Span& span, const std::u32string& name)
{
    ...
    throw ParsingException("parsing error at '" + fileName + ":" + std::to_string(token.line) + "': " + util::ToUtf8(name) + " expected:\n" + 
        util::ToUtf8(ErrorLines(span)), fileName, span);
}
...
{
    Token token = GetToken(span.start);
    ParsingException error("parsing error at '" + fileName + ":" + std::to_string(token.line) + "': " + util::ToUtf8(name) + " expected:\n" + 
        util::ToUtf8(ErrorLines(span)), fileName, span);
