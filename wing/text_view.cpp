...
module wing.text_view;

import wing.application;
import wing.edit_command;
import util;
...
{
    if (c == '.') return false;
    if (c == ' ' || util::IsPunctuation(c)) return true;
    return false;
}
...
int ReadCaretTimeoutFromRegistry()
{
    ...
    {
        std::u16string subkeyStr = util::ToUtf16("Control Panel\\Desktop");
        std::u16string valueName = util::ToUtf16("CaretTimeout");
    ...
    maxLineLength(0), maxLineIndex(0), indentSize(1), selection(), padding(), lines(), editCommandList(this), update(false), stringFormat(), ccLineNumber(0)
{
    ...
    std::u16string familyName = util::ToUtf16(fontFamilyName);
...
void TextView::PrevWord(int& line, int& col)
{
    if (Prev(line, col))
    {
        ...
        while (util::IsWhiteSpace(c) || IsWordSeparator(c))
        {
        ...
        c = GetCharAt(line, col);
        if (util::IsWhiteSpace(c) || IsWordSeparator(c))
        {
            Next(line, col);
        }
    }
...
void TextView::InsertChar(int lineIndex, int columnIndex, char32_t c)
{
    ...
    if (c == '}' && line.starts_with(std::u32string(IndentSize(), ' ')))
    {
...
void TextView::UnindentSelection()
{
    ...
    for (int i = startLineIndex; i <= endLineIndex; ++i)
    {
        ...
        if (line.starts_with(indentString))
        {
...
void TextView::ExtendSelection(const lexer::SourcePos& start, const lexer::SourcePos& end)
{
...
void TextView::OnMouseDown(MouseEventArgs& args)
{
    ...
    if (args.buttons == MouseButtons::lbutton && !lines.empty())
    {
        ...
        if (line >= 1 && line <= lines.size())
        {
            ...
            mouseSelectionStart = lexer::SourcePos(line, column);
...
void TextView::OnMouseMove(MouseEventArgs& args)
{
    ...
    if (MouseExtendSelection())
    {
        ...
        {
            FixColumn(column, line);
            mouseSelectionEnd = lexer::SourcePos(line, column);
...
void TextView::OnKeyDown(KeyEventArgs& args)
{
    ...
    if (!args.handled)
    {
        ...
        {
            lexer::SourcePos start(CaretLine(), CaretColumn());
            lexer::SourcePos end(CaretLine(), 1);
        ...
        {
            lexer::SourcePos start(CaretLine(), CaretColumn());
            lexer::SourcePos end(CaretLine(), GetLineLength(lineNumber) + 1);
            ExtendSelection(start, end);
            columnNumber = GetLineLength(lineNumber) + 1 + LineNumberFieldLength();
        ...
        {
            ResetSelection();
            lexer::SourcePos start(1, 1);
            lexer::SourcePos end(static_cast<int>(lines.size() + 1), 1);
            ExtendSelection(start, end);
            InvalidateLines(selection.start.line, selection.end.line);
            args.handled = true;
            break;
...
void TextView::Measure(Graphics& graphics)
{
    ...
    {
        ms = measureString;
        maxLineLength = static_cast<int>(util::ToUtf32(measureString).length());
        measure = true;
    }
    else
    {
        ...
        {
            ms = util::ToUtf8(*lines[maxLineIndex]);
            measure = true;
        }
    }
    if (measure)
...
void TextView::OnMouseDoubleClick(MouseEventArgs& args)
{
    ...
    ResetMouseExtendSelection();
    mouseSelectionStart = lexer::SourcePos();
    mouseSelectionEnd = lexer::SourcePos();
    SetFocus();
    ResetSelection();
    int line = 0;
    int column = 0;
    ...
    lexer::SourcePos start(line, column);
    c = GetCharAt(line, column);
    while (!util::IsWhiteSpace(c) && !IsWordSeparator(c))
    {
        if (!Next(line, column))
    ...
    lexer::SourcePos end(line, column);
    ExtendSelection(start, end);
