...
{
    std::u16string wformatName = util::ToUtf16(formatName);
...
Clipboard::Clipboard(HWND handle_) : handle(handle_)
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        {
            std::this_thread::sleep_for(std::chrono::milliseconds(util::Random() % maxWait));
            maxWait *= 2;
        }
    }
    throw WindowsException(GetLastError());
...
std::u32string Clipboard::GetStringData(const ClipboardFormat& format)
{
    ...
    else if (format.Number() == unicodeTextClipboardFormat)
    {
        ...
        return util::ToUtf32(std::u16string(ptr.get()));
    }
    else
    {
