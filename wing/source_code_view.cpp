...
void SourceCodeView::DrawLine(Graphics& graphics, int lineIndex, const PointF& origin)
{
    ...
    std::string lineNumberText = util::Format(lineNumberStr, numLineNumberDigits, util::FormatJustify::right);
    PointF pt(origin);
    ...
    for (const Token& token : tokenLine.tokens)
    {
        ...
        std::string s(util::ToUtf8(tokenStr));
...
void SourceCodeView::OnLinesChanged()
{
    ...
    numLineNumberDigits = util::Log10(n + 1);
    for (int i = 0; i < n; ++i)
    {
