...
{
    return (std::filesystem::path(WingstallRoot()) / std::filesystem::path("unicode") / std::filesystem::path("wingstall_ucd.bin")).generic_string();
}
...
{
    static_cast<uint8_t>('W'), static_cast<uint8_t>('N'), static_cast<uint8_t>('G'), static_cast<uint8_t>('_'),
    static_cast<uint8_t>('U'), static_cast<uint8_t>('C'), static_cast<uint8_t>('D'), current_wingstall_ucd_version
};
...
{
    std::string ucdFilePath = WingstallUcdFilePath();
    return ucdFilePath;
}
...
void CharacterTable::ReadHeader(BinaryStreamReader& reader)
{
    ...
    for (int i = 0; i < 7; ++i)
    {
        ...
        {
            throw UnicodeException("invalid wingstall_ucd.bin header magic: 'WNG_UCD' expected");
        }
    }
    if (magic[7] != headerMagic[7])
    {
        throw UnicodeException("invalid wingstall_ucd.bin version: version " + std::string(1, headerMagic[7]) + " expected, version " + std::string(1, magic[7]) + " read");
    }
