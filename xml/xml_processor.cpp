...
void XmlProcessor::EndStartTag(const lexer::Span& span, const lexer::SourcePos& sourcePos, const std::string& systemId)
{
...
void XmlProcessor::EndTag(const std::u32string& tagName, const lexer::Span& span, const std::string& systemId)
{
    if (tagStack.empty())
    {
        throw XmlProcessingException(GetErrorLocationStr(systemId, span) + ": end tag '" + util::ToUtf8(tagName) + "' has no corresponding start tag");
    }
    if (tagName != currentTagName)
    {
        throw XmlProcessingException(GetErrorLocationStr(systemId, span) + ": end tag '" + util::ToUtf8(tagName) + "' does not match start tag '" + util::ToUtf8(currentTagName) + "'");
    }
    std::u32string localName;
    std::u32string prefix;
    ParseQualifiedName(currentTagName, localName, prefix, span, systemId);
...
void XmlProcessor::AddAttribute(const std::u32string& attName, const std::u32string& attValue, const lexer::Span& span, const std::string& systemId)
{
    std::u32string localName;
    std::u32string prefix;
...
void XmlProcessor::EntityRef(const std::u32string& entityName, const lexer::Span& span, const std::string& systemId)
{
...
std::u32string XmlProcessor::GetNamespaceUri(const std::u32string& namespacePrefix, const lexer::Span& span, const std::string& systemId)
{
    ...
    {
        throw XmlProcessingException(GetErrorLocationStr(systemId, span) + ": namespace prefix '" + util::ToUtf8(namespacePrefix) + "' not bound to any namespace URI");
    }
}

void XmlProcessor::ParseQualifiedName(const std::u32string& qualifiedName, std::u32string& localName, std::u32string& prefix, const lexer::Span& span, const std::string& systemId)
{
    std::vector<std::u32string> parts = util::Split(qualifiedName, ':');
    if (parts.size() > 2)
    {
        throw XmlProcessingException(GetErrorLocationStr(systemId, span) + ": qualified name '" + util::ToUtf8(qualifiedName) + "' has more than one ':' character");
    }
...
std::string XmlProcessor::GetErrorLocationStr(const std::string& systemId, const lexer::Span& span) const
{
    lexer::Token token = lexer.GetToken(span.start);
