...
module xpath.xpath_lexer;

import xpath.xpath_keywords;
import xpath.xpath_tokens;
import xpath.xpath_class_map;

using namespace XPathTokens;
using lexer::INVALID_TOKEN;
using lexer::CONTINUE_TOKEN;

XPathLexer::XPathLexer(const std::u32string& content_, const std::string& fileName_, int fileIndex_) : lexer::Lexer(content_, fileName_, fileIndex_)
{
    SetKeywordMap(XPathKeywords::GetKeywordMap());
}

XPathLexer::XPathLexer(const char32_t* start_, const char32_t* end_, const std::string& fileName_, int fileIndex_) : lexer::Lexer(start_, end_, fileName_, fileIndex_)
{
    SetKeywordMap(XPathKeywords::GetKeywordMap());
}
...
int XPathLexer::NextState(int state, char32_t c)
{
    ...
    case 23:
    {
        lexer::Lexeme prevMatch = token.match;
        token.match = lexeme;
