...
module rex.token_value_parsers;

import util;

namespace rex {

void ParseHexChar(const std::string& fileName, char32_t& value, const char32_t*& p, const char32_t* e, const lexer::Token& token)
{
    if (p != e)
    {
...
char32_t ParseEscape(const std::string& fileName, const char32_t*& p, const char32_t* e, const lexer::Token& token)
{
    char32_t value = '\0';
    if (p != e && (*p == 'x' || *p == 'X'))
    {
